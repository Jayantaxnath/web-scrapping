...
import random  # Add random for sampling
import math  # Add math for ceiling function
...
for url in urls:
    try:
        ...
        # Extract filename from URL for CSV naming
        url_parts = url.split('/')
        csv_filename = url_parts[-1] + ".csv"  # Gets "internship-in-bangalore.csv"
        print(f"📄 CSV filename will be: {csv_filename}")

        # Use requests to get the initial page content with retry mechanism
        print(f"🌐 Fetching initial page: {url}")
        try:
            page_content = make_request_with_retry(url)
            page_beautify = bs4(page_content.text, "html.parser")
        except Exception as e:
            print(f"❌ Failed to fetch initial page for {url}: {e}")
            print(f"⏭️  Skipping {url} and continuing with next URL...")
        ...
        try:
            ...
            if pagination_div:
                ...
                if page_numbers:
                    total_available_pages = max(page_numbers)
                else:
                    total_available_pages = 1
            else:
                total_available_pages = 1
        except (AttributeError, IndexError):
            total_available_pages = 1  # Fallback if pagination isn't found

        # Collect all internships from all available pages
        pages_to_scrape = total_available_pages

        print(f"📊 Collection Strategy:")
        print(f"Total available pages: {total_available_pages}")
        print(f"Pages to scrape: {pages_to_scrape}")
        print(f"📈 Collecting ALL internships from all pages")

        profiles = []

        print(f"\nStarting collection from {pages_to_scrape} pages...")

        for i in range(1, pages_to_scrape + 1):
            print(f"📄 Collecting from page {i}/{pages_to_scrape}...")
            try:
                next_page_url = f"{url}/page-{i}"
                next_page_content = make_request_with_retry(next_page_url)
                ...
                # Process ALL internships from this page
                print(f"   → Found {len(big_boxes)} internships on this page")
                
                if not big_boxes:
                    print(f"   ⚠️  No internships found on page {i}, continuing...")
                    continue
                    
            except Exception as page_error:
                print(f"   ❌ Error fetching page {i}: {page_error}")
                print(f"   ⏭️  Skipping page {i} and continuing...")
                continue

            successful_internships = 0
            failed_internships = 0
            
            for box_index, box in enumerate(big_boxes, 1):
                now = datetime.now()
                ...
                try:
                    ...
                    if detail_link:
                        ...
                        try:
                            detail_page_content = make_request_with_retry(detail_page_url, max_retries=2, timeout=10)
                ...
                successful_internships += 1
            
            # Page completion summary
            total_processed = successful_internships + failed_internships
            print(f"   ✅ Page {i} completed: {successful_internships}/{total_processed} internships processed successfully")
            if failed_internships > 0:
                print(f"   ⚠️  {failed_internships} internships failed on this page")

        # Final summary for this URL
        print(f"\n🎉 Completed scraping for {url}")
        print(f"📊 Total internships collected: {len(profiles)}")
        print(f"📄 Data will be saved to: {csv_filename}")

        print(profiles)
        
        # CSV saving - always overwrite existing file
        if profiles:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    print(f"Data successfully overwritten in {csv_filename} with {len(profiles)} records")
                    break
